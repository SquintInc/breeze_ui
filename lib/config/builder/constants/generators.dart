...
String _getVarNameSuffix(final String key, final TwUnit unit) {
  final identifier = switch (unit.type) {
    UnitType.px ||
    UnitType.em ||
    UnitType.rem =>
      key == 'DEFAULT' ? '' : key.toSnakeCase(),
    UnitType.percent => _percentageVarNameSuffix(key, unit.value),
...
String _getDartLineDeclaration({
  ...
  return 'const $variableName = $constructedValue;';
}
  ...
  Future<String> generate(
    ...
    final allDeclarations = variablePrefixToValueClassName.entries.map((
      ...
      final prefixDeclarations = themeValues.entries.map((final themeValue) {
        final String varName = _variableName(
          prefix.key,
          _getVarNameSuffix(themeValue.key, themeValue.value),
        );
        final String lineDeclaration = _getDartLineDeclaration(
